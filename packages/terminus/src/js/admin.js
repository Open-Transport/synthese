...
function mlib_new_click()
{
  $('#mlib_new_modal').modal('show');
  return false;
}

function mlib_remove_click()
{
  $('#remove_title').html("de médiathèque");
  $('#remove_name').html("la médiathèque "+ $(this).attr('element_name'));
  $('#remove_form_tab_AUTOGENERATEDFIELDID').val('media_libraries');
  $('#remove_form_open_subscriptions_AUTOGENERATEDFIELDID').val(0);
  $('#remove_form_actionParamoi_AUTOGENERATEDFIELDID').val($(this).attr('element_id'));
  $('#remove_modal').modal('show');
  return false;
...
function mlib_star_click()
{
  $('[name=mlib_set_default]').submit();
  return false;
}
...
$(function(){
  ...
  $('#mlib_new_link').click(mlib_new_click);
