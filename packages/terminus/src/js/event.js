    ...
    $('#table_alternatives > div.row-fluid').each(function(){
      ...
      for(var i=0; i<message.alternative.length; ++i)
      {
        ...
        if(at.type_id == $(this).attr('type_id'))
        {
          ...
          var tinyMCEId = $(this).find("textarea").attr("id");
          tinyMCE.get(tinyMCEId).setContent(text);
          break;
        }
      }
    });
    ...
    $('#table_alternatives > div.row-fluid').each(function(){
      if(!$(this).hasClass('hide'))
      {
        for(var i=0; i<current_message.alternative.length; ++i)
        {
          ...
          {
            var tinyMCEId = $(this).find("textarea").attr("id");
            var tinyMCEContent = tinyMCE.get(tinyMCEId).getContent({format: 'text'}).replace(/\"/g,"\\\"");
            at.content = tinyMCEContent;
            break;
          }
        }
      }
...
function append_img_tag() {
  if (current_message.tags == '') {
    current_message.tags = 'img';
  } else {
    // only append if not present
    if (! (current_message.tags.indexOf(',img') > 0
           || current_message.tags.indexOf('img,') >= 0
           || current_message.tags == 'img')) {
      current_message.tags += ',img';
    }
  }
