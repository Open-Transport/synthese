...
function add_period(period)
{
  ...
  var s =
   '<div class="well form-horizontal" rank="' + period.rank +'" period_id="'+ period.id +'">' +
    '<h4>Plage horaire '+ (period.rank+1) +'<div class="pull-right"><a href="#" class="btn btn-danger btn-mini" field="remove_period">Supprimer</a></div></h4>' +
    ...
    '<div class="control-group">' +
     ...
     '<div class="controls">' +
      '<input type="text" class="input-mini" field="start_hour" placeholder="HH:MM" value="'+ period.start_hour +'" /> à ' +
      '<input type="text" class="input-mini" field="end_hour" placeholder="HH:MM" value="'+ period.end_hour +'" />' +
     '</div>' +
    '</div>' +
   '</div>';
  ...
  $('#periods').append(s);
  $('.date').datepicker();
  $('a[field=remove_period]').click(remove_period_click);
}

function remove_period_click()
{
  current_calendar.period.splice($(this).closest('div[rank]').attr('rank'), 1);
