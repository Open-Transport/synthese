...
bool gps::closeTalk()
{
	...
	try{
		...
		boost::asio::streambuf request;
		std::ostream request_stream(&request);
		request_stream << "?WATCH={\"enable\":false}";
		if(boost::asio::write(m_socket, request)>0)
		{
			bSucceeded = true;
		}
...
bool gps::updateFromGps()
{
	...
	try{
		...
		for(int i=0;i<100;i++){	// Theoriticaly should successed after two maximum 3 loops
			...
			size_t size = boost::asio::read_until(m_socket, response, "\r\n");
			std::cout << "updateFromGps just red "<< size << "bytes" << std::endl;
...
bool gps::ExtractLongitudeAndLatitudeJson(std::string &str, double &longitude, double &latitude)
{
	...
	std::cout << "read JSON buffer: str:" << str << std::endl;
	...
	{	
		...
		if(response == "TPV")
		{
			try{
				...
				std::cout << "cannot read the data" << std::endl;
				readsucceded=false;
			}
		}
	}
