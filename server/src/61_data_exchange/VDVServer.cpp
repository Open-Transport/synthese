		...
		void VDVServer::updateSYNTHESEFromServer() const
		{
			...
			try
			{
				...
				if (!plannedDataSource.get())
				{
					...
					Log::GetInstance().warn("La source des données plannifiées associée à la connexion VDV n'a pas été trouvée");					return;
				}
				...
				if (datenAbrufenAntwortResults.error != eXMLErrorNone ||
					...
					Log::GetInstance().warn("Réception d'un DatenAbrufenAntwort vide ou mal formé");					return;
				}
