...
my $sth = $dbh->prepare("SELECT id FROM t082_free_drt_areas ORDER BY id;");
$sth->execute();

# Run on id of t082_free_drt_areas looking for those with node_id 0 or 1
...
print "We found $num_id_to_change free_drt_areas to change\n";
...
open(FILE,">replace_id_t082_free_drt_areas.sql") or die"open: $!";
$cpt=0;
for ($cpt;$cpt<$num_id_to_change;$cpt++)
{
	print "Update $cpt on $num_id_to_change (".$tab_id_to_change[$cpt]." => ".$tab_new_id[$cpt].")\n";
	#0.  Column id of t082_free_drt_areas
	my $sql = "UPDATE t082_free_drt_areas SET id = $tab_new_id[$cpt] WHERE id = $tab_id_to_change[$cpt]";
	print FILE $sql.";\n";
	#1. Column area_id of t083_free_drt_time_slots
	my $sql = "UPDATE t083_free_drt_time_slots SET area_id = $tab_new_id[$cpt] WHERE area_id = $tab_id_to_change[$cpt]";
	print FILE $sql.";\n";
}
