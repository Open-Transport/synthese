...
my $sth = $dbh->prepare("SELECT id FROM t055_calendar_template_elements ORDER BY id;");
$sth->execute();

# Run on id of t055_calendar_template_elements looking for those with node_id 0 or 1
...
open(FILE,">replace_id_t055_calendar_template_elements.sql") or die"open: $!";
$cpt=0;
for ($cpt;$cpt<$num_id_to_change;$cpt++)
{
	print "Update $cpt on $num_id_to_change (".$tab_id_to_change[$cpt]." => ".$tab_new_id[$cpt].")\n";
	#0.  Column id of t055_calendar_template_elements
	my $sql = "UPDATE t055_calendar_template_elements SET id = $tab_new_id[$cpt] WHERE id = $tab_id_to_change[$cpt]";
	print FILE $sql.";\n";
}
