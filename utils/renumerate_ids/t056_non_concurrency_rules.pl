...
my $sth = $dbh->prepare("SELECT id FROM t056_non_concurrency_rules ORDER BY id;");
$sth->execute();

# Run on id of t056_non_concurrency_rules looking for those with node_id 0 or 1
...
print "We found $num_id_to_change non_concurrency_rules to change\n";
...
open(FILE,">replace_id_t056_non_concurrency_rules.sql") or die"open: $!";
$cpt=0;
for ($cpt;$cpt<$num_id_to_change;$cpt++)
{
	print "Update $cpt on $num_id_to_change (".$tab_id_to_change[$cpt]." => ".$tab_new_id[$cpt].")\n";
	#0.  Column id of t056_non_concurrency_rules
	my $sql = "UPDATE t056_non_concurrency_rules SET id = $tab_new_id[$cpt] WHERE id = $tab_id_to_change[$cpt]";
	print FILE $sql.";\n";
}
