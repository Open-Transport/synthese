...
my $sth = $dbh->prepare("SELECT id FROM t076_service_calendar_links ORDER BY id;");
$sth->execute();

# Run on id of t076_service_calendar_links looking for those with node_id 0 or 1
...
print "We found $num_id_to_change service_calendar_links to change\n";
...
open(FILE,">replace_id_t076_service_calendar_links.sql") or die"open: $!";
$cpt=0;
for ($cpt;$cpt<$num_id_to_change;$cpt++)
{
	print "Update $cpt on $num_id_to_change (".$tab_id_to_change[$cpt]." => ".$tab_new_id[$cpt].")\n";
	#0.  Column id of t076_service_calendar_links
	my $sql = "UPDATE t076_service_calendar_links SET id = $tab_new_id[$cpt] WHERE id = $tab_id_to_change[$cpt]";
	print FILE $sql.";\n";
}
