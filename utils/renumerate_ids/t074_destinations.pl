...
my $sth = $dbh->prepare("SELECT id FROM t074_destinations ORDER BY id;");
$sth->execute();

# Run on id of t074_destinations looking for those with node_id 0 or 1
...
print "We found $num_id_to_change destinations to change\n";
...
open(FILE,">replace_id_t074_destinations.sql") or die"open: $!";
$cpt=0;
for ($cpt;$cpt<$num_id_to_change;$cpt++)
{
	print "Update $cpt on $num_id_to_change (".$tab_id_to_change[$cpt]." => ".$tab_new_id[$cpt].")\n";
	#0.  Column id of t074_destinations
	my $sql = "UPDATE t074_destinations SET id = $tab_new_id[$cpt] WHERE id = $tab_id_to_change[$cpt]";
	print FILE $sql.";\n";
	#1. Column direction_id of t009_lines
	my $sql = "UPDATE t009_lines SET direction_id = $tab_new_id[$cpt] WHERE direction_id = $tab_id_to_change[$cpt]";
	print FILE $sql.";\n";
}
