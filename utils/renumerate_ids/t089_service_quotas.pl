...
my $sth = $dbh->prepare("SELECT id FROM t089_service_quotas ORDER BY id;");
$sth->execute();

# Run on id of t089_service_quotas looking for those with node_id 0 or 1
...
print "We found $num_id_to_change service_quotas to change\n";
...
open(FILE,">replace_id_t089_service_quotas.sql") or die"open: $!";
$cpt=0;
for ($cpt;$cpt<$num_id_to_change;$cpt++)
{
	print "Update $cpt on $num_id_to_change (".$tab_id_to_change[$cpt]." => ".$tab_new_id[$cpt].")\n";
	#0.  Column id of t089_service_quotas
	my $sql = "UPDATE t089_service_quotas SET id = $tab_new_id[$cpt] WHERE id = $tab_id_to_change[$cpt]";
	print FILE $sql.";\n";
}
