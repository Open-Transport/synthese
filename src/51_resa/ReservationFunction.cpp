...
namespace synthese
{
	...
	namespace resa
	{
		...
		util::ParametersMap ReservationFunction::run( std::ostream& stream, const Request& request ) const
		{
			...
			map.insert("customerId", resa.get() ? lexical_cast<string>(resa->getCustomerUserId()) : string());
			...
			map.insert("customerName", resa.get() ? resa->getCustomerName() : string());
			map.insert("customerPhone", resa.get() ? resa->getCustomerPhone() : string());
			map.insert("status", resa.get() ? ResaModule::GetStatusText(resa->getStatus()) : "N.A.");
			map.insert("canBeCancelled", resa.get() ? lexical_cast<string>(second_clock::local_time() < resa->getReservationDeadLine() && resa->getCancellationTime().is_not_a_date_time()) : "0");
			map.insert("seats", resa.get() ? lexical_cast<string>(resa->getSeats()) : "0");
			if(resa.get() && !resa->getCancellationTime().is_not_a_date_time())
			{
				map.insert("cancellationDateTime", posix_time::to_iso_extended_string(resa->getCancellationTime()));
			}
			if(resa.get())
			{
				BOOST_FOREACH(const Reservation* r, resa->getReservations())
				{
					...
					resaMap->insert("departurePlaceName", r->getDeparturePlaceName());
					resaMap->insert("departureDateTime", posix_time::to_iso_extended_string(r->getDepartureTime()));
					resaMap->insert("arrivalPlaceName", r->getArrivalPlaceName());
					resaMap->insert("arrivalDateTime", posix_time::to_iso_extended_string(r->getArrivalTime()));
					resaMap->insert("lineNumber", r->getLineCode());
