...
#define FIELD_DEFINITION_OF_OBJECT(N, F, VF) 	template<> const Field ObjectFieldDefinition<N>::FIELD = Field(F, SQL_INTEGER); \
	template<> const bool ObjectFieldDefinition<N>::EXPORT_CONTENT_AS_FILE = false; \
	template<> const Field ObjectFieldDefinition<N::Vector>::FIELD = Field(VF, SQL_TEXT); \
	template<> const bool ObjectFieldDefinition<N::Vector>::EXPORT_CONTENT_AS_FILE = false;
#define FIELD_DEFINITION_OF_TYPE(N, F, T) 	template<> const Field ObjectFieldDefinition<N>::FIELD = Field(F, T); \
	template<> const bool ObjectFieldDefinition<N>::EXPORT_CONTENT_AS_FILE = false;
