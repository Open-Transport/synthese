...
namespace synthese
{
	...
	void ObjectField<Parent, Parent::Type>::GetLinkedObjectsIds(
	...
	void ObjectField<Parent, Parent::Type>::LoadFromRecord(
		Parent::Type fieldObject,
		ObjectBase& object,
		const Record& record,
		const Env& env
	){
	...
	void ObjectField<Parent, Parent::Type>::SaveToParametersMap(
	...
	void ObjectField<Parent, Parent::Type>::SaveToParametersMap(
		...
		if(fieldObject && dynamic_cast<const Registrable*>(fieldObject))
		{
			map.insert(prefix + FIELD.name, dynamic_cast<const Registrable*>(fieldObject)->getKey());
		}
	}
