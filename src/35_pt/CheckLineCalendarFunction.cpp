		...
		void CheckLineCalendarFunction::run(
			...
			if(	!_line.get()
				...
				if(	!_line->getCalendarTemplate()
					...
					{
						stream << "0\nOK Line "+ _line->getShortName() +" runs the "+ lexical_cast<string>(_duration.days()) +" next days of "+ _line->getCalendarTemplate()->getText() +"|days="+ lexical_cast<string>(runDays.size()) + "\n";
					}
					else
					{
						stream << "2\nKO Line "+ _line->getShortName() +" does not run then "+ lexical_cast<string>(_duration.days()) +" days of "+ _line->getCalendarTemplate()->getText() +"|days="+ lexical_cast<string>(runDays.size()) + "\n";
					}
				}
			}
