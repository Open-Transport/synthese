...
namespace synthese
{
	...
	namespace pt
	{
		...
		const string CarPostalFileFormat::Importer_::FILE_GLEIS = "gleis";
		...
		const string CarPostalFileFormat::Importer_::PARAMETER_IMPORT_FULL_SERVICES = "import_full_services";
	}

	namespace impex
	{
		template<> const MultipleFileTypesImporter<CarPostalFileFormat>::Files MultipleFileTypesImporter<CarPostalFileFormat>::FILES(
			CarPostalFileFormat::Importer_::FILE_GLEIS.c_str(),
	...
	namespace pt
	{
				...
				while(getline(inFile, line))
				{
					...
					if(line[0] == '@')
					{
						itService = _gleisMap.insert(
							make_pair(
								make_tuple(line.substr(3, 5), line.substr(9, 6), lexical_cast<size_t>(line.substr(16, 2))),
					...
					else
					{
						itService->second.insert(
							make_pair(
								line.substr(0, 7),
								trim_copy(line.substr(10, 6))
						)	);
					}
				}
			}
			// 0 : Coordinates
			else if(key == FILE_KOORD)
			{
				string line;
				...
				error = (!_nonLinkedBahnhofs.empty() || _showStopsOnly) && !_importFullServices;
			...
			else if (key == FILE_ZUGDAT)
			{
				...
				while(getline(inFile, line))
				{
					...
					if(line.substr(0,2) == "*Z")
					{
						...
						version = lexical_cast<size_t>(trim_copy(line.substr(16, 2)));
						stops.clear();
						departures.clear();
						arrivals.clear();
					...
					{
						transportMode = PTFileFormat::GetTransportMode(
							transportModes,
							trim_copy(line.substr(3, 3)),
							os
						);
					}
					else if(line.substr(0,1) != "*")
					{
						string stopCode(line.substr(0,7));
						...
						else
						{
							...
							GleisMap::const_iterator itGleis(
								_gleisMap.find(
									make_tuple(serviceNumber, lineNumber, version)
							)	);
							if(itGleis != _gleisMap.end())
							{
								...
								{
									vector<set<StopPoint*>::iterator> toRemove;
									for(set<StopPoint*>::iterator itStopPoint(stopsSet.begin()); itStopPoint != stopsSet.end(); ++itStopPoint)
									{
										if(!(*itStopPoint)->hasCodeBySource(_dataSource, itStopArea->second))
										{
											toRemove.push_back(itStopPoint);
										}
									}
									BOOST_FOREACH(set<StopPoint*>::iterator itToRemove, toRemove)
									{
										stopsSet.erase(itToRemove);
									}
									if(stopsSet.empty())
									{
										os << "WARN : stop " << line << " / gleis " << itStopArea->second << " not found<br />";
										serviceMustBeAvoided = true;
									}
								}
							}
		...
		void CarPostalFileFormat::Importer_::displayAdmin(
			...
			stream << t.cell("Importer les services compets", t.getForm().getOuiNonRadioInput(PARAMETER_IMPORT_FULL_SERVICES, _importFullServices));
			stream << t.close();
		}
