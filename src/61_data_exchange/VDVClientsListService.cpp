...
namespace synthese
{
	...
	namespace data_exchange
	{
		...
		const string VDVClientsListService::TAG_SUBSCRIPTION = "subscription";
		...
		ParametersMap VDVClientsListService::run(
			...
			BOOST_FOREACH(const DataExchangeModule::VDVClients::value_type& client, DataExchangeModule::GetVDVClients())
			{
				...
				// Client attributes
				client.second->toParametersMap(*clientPM);

				// Subscription
				BOOST_FOREACH(const VDVClient::Subscriptions::value_type& subscription, client.second->getSubscriptions())
				{
					shared_ptr<ParametersMap> subscriptionMap(new ParametersMap);
					subscription.second->toParametersMap(*subscriptionMap);
					clientPM->insert(TAG_SUBSCRIPTION, subscriptionMap);
				}
