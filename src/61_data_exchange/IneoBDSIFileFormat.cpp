...
namespace synthese
{
	...
	namespace data_exchange
	{
					...
					else
					{
						...
						else
						{
							...
							else
							{
								...
								if(_alreadyAutoGeneratedStopPoints.contains(mnemol))
								{
									stopPoint = *(_alreadyAutoGeneratedStopPoints.get(mnemol)).begin();
									_logLoadDetail(
										"STOP",mnemol,name,stopPoint->getKey(),stopPoint->getConnectionPlace()->getFullName() + "/" + stopPoint->getName(), string(), string(), "OK"
									);
								}
								else
								{
									...
									if (tryToRecognizeStopPoint)
									{
										...
										if (!stopAreaForCreation)
										{
											...
											_alreadyAutoGeneratedStopPoints.add(*stopPoint);
		...
		IneoBDSIFileFormat::Importer_::Importer_(
			...
			_alreadyAutoGeneratedStopPoints(*import.get<DataSource>(), Env::GetOfficialEnv()),
