...
namespace synthese
{
	...
	namespace vinci
	{
		...
		AdminInterfaceElement::PageLinks VinciCustomerAdminInterfaceElement::getSubPagesOfParent( const PageLink& parentLink , const AdminInterfaceElement& currentPage		, const server::FunctionRequest<admin::AdminRequest>* request
			) const
		{
		...
		std::string VinciCustomerAdminInterfaceElement::getTitle() const
		{
			return
				_contract.get()
					? (_user.get() ? "Client " + _user->getSurname() + " " + _user->getName() : VinciContract::DEFAULT_NAME)
					: DEFAULT_TITLE;
			;
		}
		...
		{
			return _contract.get() ? QueryString::PARAMETER_OBJECT_ID : string();
		}

		std::string VinciCustomerAdminInterfaceElement::getParameterValue() const
		{
			return _contract.get() ? Conversion::ToString(_contract->getKey()) : string();
		}

		std::string VinciCustomerAdminInterfaceElement::getIcon() const
		{
			return _user.get() ? ICON : "basket.png";
		}
	}
}
