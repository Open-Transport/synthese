...
/*! \brief Chargement d'un fichier d'élément d'interface
  ...
  \date 2002-2005
*/
bool cFichierObjetInterfaceStandard::Charge(cInterface_Objet_AEvaluer_PageEcran& __Objet, const Interface& __Interface)
{
    // Variables
	...
	case ELEMENTSINTERFACEFORMATLIGNETexte:
	    __Objet.Chaine(
		new cInterface_Objet_Element_Parametre_TexteConnu(
		    _Format->Extrait(Tampon, ELEMENTSINTERFACEFORMATCOLONNECommandePourElementStatique).Texte ()
		    ));
	    break;
	...
	case -TYPELien:
	{
	    ...
	    __ParametresDonnees.InterpreteTexte(cInterface_Objet_AEvaluer_ListeParametres(), _Format->Extrait(Tampon, ELEMENTSINTERFACEFORMATCOLONNECommande).Texte ());
	...
	{
	    cInterface_Objet_Element_Bibliotheque* __EI = new cInterface_Objet_Element_Bibliotheque(_Format->GetNombre(Tampon, INTERFACESFORMATCOLONNENumero), _Format->Extrait(Tampon, ELEMENTSINTERFACEFORMATCOLONNECommande).Texte ());
	    __Objet.Chaine(__EI);
	}
	break;
