...
namespace synthese
{
	...
	/**	@defgroup m34Actions 54 Actions
		...
		Un systÃšme de téléaffichage basé sur SYNTHESE repose sur les principes suivants :
			...
			- les sources dites <i>théoriques</i> : elles proviennent de fichiers mis Ã  jour par saisie manuelle ou par import informatique
			- les sources dites <i>temps réel</i> : saisies manuelles dans la console d'administration (ex : saisie de messages...) ou réceptions de données transmises automatiquement (ex : contrÃŽles de supervision issues du client de supervision, import modifications horaires SAE...)
			- le téléaffichage est un service du systÃšme SYNTHESE comme un autre : il exploite la base de données Ã  chaud et fabrique des objets résultats (une liste de départs ou d'arrivée) qui sont transmis au module d'interface, chargé de les diffuser selon un langage défini
			- le module d'interface permet de définir le format des sorties sous forme de données d'entrée, dans une grande liberté de choix. Les formats de sortie peuvent Ãªtre définis en HTML pour intégration Ã  une charte graphique, en XML pour communiquer avec d'autres applications, ou encore selon un langage propriétaire pour piloter des dispositifs de téléaffichage matériels

		Le schéma suivant représente l'architecture cible d'un systÃšme de téléaffichage basé sur la solution SYNTHESE :

		@image latex 1195515_1_000.eps "Architecture cible d'un systÃšme de téléaffichage SYNTHESE" height=10cm
