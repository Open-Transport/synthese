...
LPTSTR toXMLStringFast( LPTSTR *dest, int *destSz, LPCTSTR source )
{
    ...
    if ( l > *destSz )
    { *destSz = l; *dest = ( LPTSTR ) realloc( *dest, l * sizeof( TCHAR ) ); }
    return toXMLString( *dest, source );
}
...
LPCTSTR XMLNode::getError( XMLError error )
{
    switch ( error )
    {
        ...
        case eXMLErrorTagNotFound:
            return _T( "First Tag not found" );
    };
    return _T( "Unknown" );
}
...
static inline void charmemset( LPTSTR dest, TCHAR c, int l ) { while ( l-- )
        * ( dest++ ) = c; }
// private:
...
void XMLNode::destroyCurrentBuffer( XMLNodeData *d )
{
    ...
    if ( d->ref_count == 0 )
    {
        ...
        { d->pChild[ i ].d->pParent = NULL; destroyCurrentBuffer( d->pChild[ i ].d ); }
        free( d->pChild );
        for ( i = 0; i < d->nText; i++ )
            free( ( void* ) d->pText[ i ] );
        free( d->pText );
...
LPCTSTR XMLNode::getName() { if ( !d )
        return NULL; return d->lpszName; }
int XMLNode::nText() { if ( !d )
        return 0; return d->nText; }
int XMLNode::nChildNode() { if ( !d )
        return 0; return d->nChild; }
int XMLNode::nAttribute() { if ( !d )
        return 0; return d->nAttribute; }
int XMLNode::nClear() { if ( !d )
        return 0; return d->nClear; }
XMLClear XMLNode::getClear ( int i ) { if ( !d )
        return emptyXMLClear; if ( i >= d->nClear )
        return emptyXMLClear; return d->pClear[ i ]; }
XMLAttribute XMLNode::getAttribute ( int i ) { if ( !d )
        return emptyXMLAttribute; if ( i >= d->nAttribute )
        return emptyXMLAttribute; return d->pAttribute[ i ]; }
LPCTSTR XMLNode::getText ( int i ) { if ( !d )
        return NULL; if ( i >= d->nText )
        return NULL; return d->pText[ i ]; }
XMLNode XMLNode::getChildNode ( int i ) { if ( !d )
        return emptyXMLNode; if ( i >= d->nChild )
        return emptyXMLNode; return d->pChild[ i ]; }
char XMLNode::isDeclaration( ) { if ( !d )
        return 0; return d->isDeclaration; }
