        ...
        case ELEMENTINTERFACEFicheHoraire:
            ...
            else
            {
                ...
                /*
                  ...
                  synthese::time::DateTime debutPrem = curET->MomentDepart();
                   ...
                   else
                   {
                    for (; curDP!=NULL; curDP=curDP->Suivant())
                    {
                     ...
                     if (curDP->getSitPert() == NULL)
                      curDP->getMomentPrevu().getHour ().toString(pCtxt);
                     else
                      curDP->getMomentReel().getHour ().toString(pCtxt);
                   ...
                   while (curDP!=NULL)
                   {
                    ...
                    for (curGLA = curDP->getGareLigne()->getArriveeSuivante(); curGLA != NULL; curGLA = curGLA->getArriveeSuivante())
                     ...
                     {     
                      ...
                      pCtxt << " (";
                      curGLA->CalculeArrivee(curDP).getHour ().toString(pCtxt);
                      pCtxt << ") ";
                     }
                    pCtxt << "\n";
        ...
        case ELEMENTINTERFACEFeuilleRoute:
            {
                ...
                for ( const cElementTrajet * __ET = __Trajet->PremierElement(); __ET != NULL; __ET = __ET->getSuivant() )
                {
                    ...
                    if ( __ET->getLigne() ->Materiel() ->Code() != MATERIELPied )
                    {
                        ...
                        // MJ review __ParametresLigne << __ET->MomentDepart().getHour (); //0
                        if ( __Trajet->getAmplitudeServiceContinu() )
                        {
                            tempMoment = __ET->MomentDepart();
                            tempMoment += __Trajet->getAmplitudeServiceContinu();
                            // MJ review __ParametresLigne << tempMoment.getHour ();  //1
                        }
                        else
                            __ParametresLigne << "";      //1

                        // 2/3 Heures d'arriv�e
                        // MJ review __ParametresLigne << __ET->MomentArrivee().getHour (); //2
                        if ( __Trajet->getAmplitudeServiceContinu() )
                        {
                            tempMoment = __ET->MomentArrivee();
                            tempMoment += __Trajet->getAmplitudeServiceContinu();
                            // MJ review __ParametresLigne << tempMoment.getHour (); //3
                            finLigne = tempMoment;
                        }
                        else
                            __ParametresLigne << "";     //3
                        ...
                        if ( __ResaOuverte )
                        {
			  // MJ review__ParametresLigne << __ET->getLigne() ->GetResa() ->momentLimiteReservation( __ET->getService(), __ET->MomentDepart() ); //14
                        ...
                        // MJ review__ParametresDescente << __ET->MomentArrivee().getHour (); //6
                        if ( __Trajet->getAmplitudeServiceContinu() )
                        {
                            tempMoment = __ET->MomentArrivee();
                            tempMoment += __Trajet->getAmplitudeServiceContinu();
                            // MJ review__ParametresDescente << tempMoment.getHour (); //7
                        }
                        else
        ...
        case EI_BIBLIOTHEQUE_Trajet_Duree:
            {
                ...
                //1 : Dur�e < 1h ?
                // MJ review__Parametres << ( __Trajet->getDuree() < synthese::time::MINUTES_PER_HOUR ? "1" : "" );

                //2 : Nombre d'heures
                // MJ review __Parametres << ( __Trajet->getDuree() / synthese::time::MINUTES_PER_HOUR );

                //3 : Nombre de minutes
                __Parametres << ( __Trajet->getDuree() % synthese::time::MINUTES_PER_HOUR );

                //4 : Nombre de minutes sup�rieur � 10 ou inf�rieur � 60 ?
                __Parametres << ( __Trajet->getDuree() < synthese::time::MINUTES_PER_HOUR || __Trajet->getDuree() % synthese::time::MINUTES_PER_HOUR >= 10 ? "1" : "" );
                ...
                else if ( __Balise.Compare( "select" ) )  // Cas champ select
                {
                    ...
                    if ( !__Type.Taille() )
                    {
                        if ( __Champ.Compare( "date" ) )
                        {
                            ...
                            const cEnvironnement * __Environnement = __Site->getEnvironnement();
                            synthese::time::Date DateDefaut = __Environnement->dateInterpretee( _Parametres[ ELEMENTINTERFACEInputHTMLValeurDefaut ] ->Texte( __Parametres ) );
                            synthese::time::Date DateMin = __Environnement->dateInterpretee( _Parametres[ ELEMENTINTERFACEInputHTMLListeDatesMin ] ->Texte( __Parametres ) );
