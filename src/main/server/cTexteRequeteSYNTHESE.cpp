...
/*! \brief Accesseur à un paramètre de type date
 ...
 \date 2005
*/
synthese::time::Date cTexteRequeteSYNTHESE::getDate( const char* __NomParam )
{
    synthese::time::Date __Date;

    if ( checkParam( __NomParam ) )
        __Date = std::string (getTexte( __NomParam ).Texte ());
    else
        __Date.updateDate( synthese::time::TIME_UNKNOWN );
...
/*! \brief Accesseur à un paramètre de type moment
 ...
 \date 2005
*/
synthese::time::DateTime cTexteRequeteSYNTHESE::getMoment( const char* __NomParam )
{
    synthese::time::DateTime __Moment;

    if ( checkParam( __NomParam ) )
        __Moment = std::string (getTexte( __NomParam ).Texte ());
    else
        __Moment.updateDateTime( synthese::time::TIME_UNKNOWN );
...
int cTexteRequeteSYNTHESE::ConvertChar( const char c, char* Tampon ) const
{
    switch ( c )
    {
        case '+':
