...
cAssociator::cAssociator( const char *netfile )
{
    ...
    if ( ( _hdr.stamp & 0xFF ) == __MLP_ID__ )
    {
        ...
        _nbinput = 0;
        while ( _lyr[ _nbinput ].name[ 0 ] == '+' )
            _nbinput++;
        _nboutput = _nbinput;
        while ( _lyr[ _nboutput ].name[ 0 ] != '-' )
            _nboutput++;
...
void cAssociator::compute( const uint16 entry )
{
    ...
    if ( _lyr[ entry ].name[ 0 ] != '+' )
    {
        ...
        for ( ; pn < pnstop; pn++ )
        {
            ...
            for ( ; pk < pkstop; pk++ )
                if ( pk->weight > 1 )
                    sumvalue += _nde[ pk->from ].value * pk->weight;
                else
                    sumvalue += _nde[ pk->from ].value;
            if ( pn->link_length )
...
cAssocResult cAssociator::MatchCity( uint16 maxsize )
{
    ...
    while ( maxsize-- && cont )
    {
        ...
        if ( cont )
            eval.delta = eval.score - ( *it ) ->value;
        else
            eval.delta = 0;
