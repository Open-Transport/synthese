...
class cFormatFichier
{
	...
	tIndex		vNombreLignes;		//!< Nombre d'identificateurs de types de lignes

public:
	//! \name Calculateurs
	//@{
	cTexte	Extrait(const cTexte& Texte, tIndex n, int NumeroColonne=0)		const;
	cTexte	ExtraitComplet(const cTexte& Texte, tIndex n)				const;
	int		GetColonneLargeur(tIndex n)								const;
	int		GetColonnePosition(tIndex n, int NumeroColonne=0)				const;
	int		GetNombre(const cTexte& Texte, tIndex n, int NumeroColonne=0)	const;
	int		LireFichier(ifstream& Fichier, char* Tampon)					const;
	int		LireFichier(ifstream& Fichier, cTexte& Tampon)				const;
	int		Longueur(const cTexte& Texte, tIndex n)						const;
	//@}
