                ...
                case SITESFORMATLIGNEDateDebut:
                    {
                        ...
                        if ( txtDate.Taille() <= 0 || txtDate[ 0 ] == ' ' )
                            dateDebut.updateDate( synthese::time::TIME_MAX );
                        else
                            dateDebut = std::string (txtDate.Texte ());
                ...
                case SITESFORMATLIGNEDateFin:
                    {
                        ...
                        if ( txtDate.Taille() <= 0 || txtDate[ 0 ] == ' ' )
                            dateFin.updateDate( synthese::time::TIME_MIN );
                        else
                            dateFin = std::string (txtDate.Texte ());
...
bool cFichierJoursCirculation::JCExecuterCommande( cJC& JC, cJC::InclusionType Sens, cTexte& Tampon, cEnvironnement* __Environnement )
{
    ...
    cTexte NomFichier2;
    synthese::time::Date Date1;
    synthese::time::Date Date2;
    int Pas;
        ...
        case JCFORMATLIGNEAjoutDate:
            switch ( Tampon[ 1 ] )
            {
                ...
                default:
                    //if (Chargement)
                    //{
		  Date1 = std::string (Tampon.Extrait( 1, 8 ).Texte());
		  if ( Tampon.Taille() > 9 && Tampon[ 9 ] == '/' )
                    {
                        Date2 = std::string (Tampon.Extrait( 10 ).Texte ());
    ...
    */
    ...
    bool cFichierPointsArret::Charge( cEnvironnement* __Environnement )
    {
        ...
        while ( PasTermineSousSection )
        {
            if ( ProchaineSection( Tampon, PAFORMATLIGNEArretLogique ) )
            {
                ...
                {
                    case 'C':
                        curNiveauCorrespondance = LogicalPlace::CorrAutorisee;
                        break;
                    case 'Q':
                        curNiveauCorrespondance = LogicalPlace::CorrRecommandeeCourt;
                        break;
                    case 'R':
                        curNiveauCorrespondance = LogicalPlace::CorrRecommandee;
                        break;
                    case 'S':
            ...
            while ( PasTermineElement )
            {
                switch ( LireLigneFormat( Tampon ) )
                {
                    ...
                    case PAFORMATLIGNEAlerteDebut:
                        {
                            ...
                            if ( dateDebut.Taille() <= 0 || dateDebut[ 0 ] == ' ' )
                                momentDebut.updateDateTime( 'm', 'm', __Environnement->PremiereAnnee() );
                            else
                    ...
                    case PAFORMATLIGNEAlerteFin:
                        {
                            ...
                            if ( dateFin.Taille() <= 0 || dateFin[ 0 ] == ' ' )
                                momentFin.updateDateTime( 'M', 'M', __Environnement->DerniereAnnee() );
                            else
                    ...
                    case PAFORMATLIGNEDescription:
                        {
                            ...
                            int Position = Tampon.RechercheOccurenceGauche( '(', 1
                    ...
                    case PAFORMATLIGNEArretPhysique:
                        // Cr�ation du quai
                        curNumeroArretPhysique = ( int ) _Format->GetNombre( Tampon, PAFORMATCOLONNEIndexArretPhysique );
    ...
    int cFichier::NombreElementsAAllouer()
    {
