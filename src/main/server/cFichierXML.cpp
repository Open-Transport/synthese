...
cFichierXML::chargeDonneesRoutesCommune ( std::istream& xmlStream, cEnvironnement& env )
{
    ...
    for ( int i = 0; i < nbRoutes; ++i )
    {
        ...
        if ( routeType == ROUTE_TYPE_AUTOROUTE )
            type = Road::MOTORWAY;
        else if ( routeType == ROUTE_TYPE_ATTR_ROUTE_CHAUSSEES_SEPAREES )
            type = Road::MEDIAN_STRIPPED_ROAD;
        else if ( routeType == ROUTE_TYPE_ATTR_AXE_PRINCIPAL )
            type = Road::PRINCIPLE_AXIS;
        else if ( routeType == ROUTE_TYPE_ATTR_AXE_SECONDAIRE )
            type = Road::SECONDARY_AXIS;
        else if ( routeType == ROUTE_TYPE_ATTR_PONT )
            type = Road::BRIDGE;
        else if ( routeType == ROUTE_TYPE_ATTR_RUE )
            type = Road::STREET;
        else if ( routeType == ROUTE_TYPE_ATTR_RUE_PIETONNE )
            type = Road::PEDESTRIAN_STREET;
        else if ( routeType == ROUTE_TYPE_ATTR_BRETELLE )
            type = Road::ACCESS_ROAD;
        else if ( routeType == ROUTE_TYPE_ATTR_VOIE_PRIVEE )
            type = Road::PRIVATE_WAY;
        else if ( routeType == ROUTE_TYPE_ATTR_CHEMIN )
            type = Road::PEDESTRIAN_PATH;
        else if ( routeType == ROUTE_TYPE_ATTR_TUNNEL )
            type = Road::TUNNEL;
        else if ( routeType == ROUTE_TYPE_ATTR_ROUTE_SECONDAIRE )
            type = Road::HIGHWAY;
...
cFichierXML::chargeModaliteResa ( XMLNode& node, cEnvironnement& env )
{
    ...
    //! \todo heure ?? addmresa->SetDelaiMinHeureMax(synthese::time::Hour (...));
...
cFichierXML::chargeAxe ( XMLNode& node, cEnvironnement& env )
{
    ...
    for ( int i = 0; i < nbLignes; ++i )
    {
        ...
        {
            ligne->setAttente ( k, int( attente ) );
        }
...
cFichierXML::chargeSite ( XMLNode& node, SYNTHESE& synthese )
{
    ...
    if ( txtDateDebut.Taille() <= 0 || txtDateDebut[ 0 ] == ' ' )
        dateDebut.setDate( synthese::time::TIME_MAX );
    else
    ...
    if ( txtDateFin.Taille() <= 0 || txtDateFin[ 0 ] == ' ' )
        dateFin.setDate( synthese::time::TIME_MAX );
    else
        dateFin = txtDateFin;
