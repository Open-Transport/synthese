...
/** Résultat */
typedef std::pair<size_t, Score> Result;

/** Clé */
typedef std::string Key;

/** ID inconnu */
static const std::size_t UNKNOWN_INDEX = -1;
...
class Interpretor
{
    ...
    public:
        ...
        */
        void add
            ( const Key& key, const T& element, size_t id )
        {
        ...
        */
        std::set
            <Result> search( const Key& key, size_t size ) const
            {
                std::set
                    <Result> result;
