...
protected:
	...
	tIndex	_TailleMax;					//!< Taille de l'espace allou�
	tIndex	_Taille;					//!< Taille de l'espace utilis�
	
	virtual void	Construit(tIndex __AncienneTailleMax);
...
public:
	//! \name Calculateurs
	//@{
	bool		IndexValide(tIndex)									const;
	tIndex		Taille()											const;
	C*			GetElement(tIndex __Index = TABLEAU_ELEMENT_DEFAUT)	;
	C&			getElement(tIndex __Index = TABLEAU_ELEMENT_DEFAUT)	;
	...
	//!	\name Modificateurs
	//@{
	bool					AlloueSiBesoin(tIndex __TailleDemandee, bool __Relatif=false);
	cTableauDynamique<C>&	Vide(tIndex __Position=0);
	tIndex					SetElement(C&, tIndex __i = INCONNU, bool __Finalisation=true);
	tIndex					SetElement(const C&, tIndex __i = INCONNU, bool __Finalisation=true);
	tIndex					CopieElement(tIndex __IndexSource, tIndex __IndexDestination = INCONNU);
	tIndex					Active(tIndex __i = INCONNU);
	cTableauDynamique<C>&	operator+=(C&);
	//void					Trie(int (__cdecl *compare )(const void *, const void *));
	//@}

	
	cTableauDynamique<C>(tIndex __TailleMax=0, bool __AugmentationParDoublement=true);
	virtual ~cTableauDynamique<C>();

	// Recherche d'un �l�ment
	tIndex Recherche(const C& __Element) const
	{
		for (tIndex __i=0; __i<Taille(); __i++)
	...
	*/
	tIndex AddElementSiInexistant(C& __Element)
	{
		tIndex __i;
...
{
protected:
	void	Construit(tIndex __AncienneTailleMax);
	void	Detruit();
};
...
*/
template <class C>
inline tIndex cTableauDynamiquePointeurs<C>::NombreReel() const
{
	tIndex __NombreElements = 0;

	for (tIndex __i = 0; __i < this->Taille(); __i++)
...
template <class C>
inline void cTableauDynamiqueObjets<C>::Construit(tIndex __AncienneTailleMax)
{
...
template <class C>
inline void cTableauDynamique<C>::Construit(tIndex __AncienneTailleMax)
{
...
*/
template <class C>
inline cTableauDynamique<C>::cTableauDynamique(tIndex __TailleMax, bool __AugmentationParDoublement)
{
	_Element = NULL;
	_Taille = 0;
...
*/
template <class C>
inline bool cTableauDynamique<C>::IndexValide(tIndex __Index) const
{
...
/*!	\brief Accesseur pointeur vers �l�ment
	...
	\return L'adresse de l'�l�ment s'il existe, NULL sinon
	\author Hugues Romain
	\date 2005
*/
template <class C>
inline C* cTableauDynamique<C>::GetElement(tIndex __Index) 
{
...
*/
template <class C>
bool cTableauDynamique<C>::AlloueSiBesoin(tIndex __TailleDemandee, bool __Relatif)
{
	// Taille n�cessaire
	tIndex __TailleTotaleDemandee = __TailleDemandee;
	if (__Relatif)
	...
	tIndex __AncienneTailleMax = _TailleMax;
...
*/
template <class C>
cTableauDynamique<C>& cTableauDynamique<C>::Vide(tIndex __Position)
{
	if (_Element != NULL && __Position >= 0 && __Position <= _TailleMax)
	{
		_Taille = __Position;
		Finalise();
	}
...
*/
template <class C>
tIndex cTableauDynamique<C>::SetElement(C& __Valeur, tIndex __Index, bool __Finalisation)
{
 ...
 */
template <class C>
tIndex cTableauDynamique<C>::SetElement(const C& __Valeur, tIndex __Index, bool __Finalisation)
{
	/*!
...
*/
template <class C>
tIndex cTableauDynamique<C>::CopieElement(tIndex __IndexSource, tIndex __IndexDestination)
{
...
*/
template <class C>
inline tIndex cTableauDynamique<C>::Active(tIndex __Index)
{
	// Cas index inconnu
	if (__Index < 0)
	{
