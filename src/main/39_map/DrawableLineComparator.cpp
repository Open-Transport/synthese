    ...
    int index) const
{
    ...
    const Point& vs0_1 = points[index];
    const Point& vs1_1 = points[index + 1];
    const Point& vs2_1 = points[index + 2];
    ...
    int index) const
{
    ...
    const Point& vs0_1 = points[index - 2];
    const Point& vs1_1 = points[index - 1];
    const Point& vs2_1 = points[index];
...
DrawableLineComparator::calculateStartAngles (
	const std::vector<Point>& points1, int index1,
	const std::vector<Point>& points2, int index2) const
{
...
std::pair<double, double>
DrawableLineComparator::calculateEndAngles (const std::vector<Point>& points1, int index1,
					    const std::vector<Point>& points2, int index2) const
{
    assert (points1.size () >= 2);
    assert (points2.size () >= 2);
...
//	std::cerr << ((long) bl1) << " < " << ((long) bl2) << " = " << 0 << std::endl;
    ...
    std::vector<Point> points1 = bl1->getFuzzyfiedPoints ();
    std::vector<Point> points2 = bl2->getFuzzyfiedPoints ();
