...
namespace synthese
{
	...
	namespace util
	{
		template<> const string FactorableTemplate<DBTableSync,TimetableRowGroupItemTableSync>::FACTORY_KEY("55.10 Timetable items");
	}
	...
	namespace db
	{
		...
		template<> void DBDirectTableSyncTemplate<TimetableRowGroupItemTableSync,TimetableRowGroupItem>::Load(
		...
		template<> void DBDirectTableSyncTemplate<TimetableRowGroupItemTableSync,TimetableRowGroupItem>::Unlink(
			TimetableRowGroupItem* obj
		){
			obj->unlink();
		}
		...
		template<> bool DBTableSyncTemplate<TimetableRowGroupItemTableSync>::CanDelete(
			...
			Env env;
			shared_ptr<const TimetableRowGroupItemTableSync::ObjectType> object(TimetableRowGroupItemTableSync::Get(object_id, env));
		...
		template<> void DBTableSyncTemplate<TimetableRowGroupItemTableSync>::BeforeDelete(
			...
			Env env;
			shared_ptr<const TimetableRowGroupItemTableSync::ObjectType> object(TimetableRowGroupItemTableSync::Get(id, env));
