...
namespace synthese
{
	...
	namespace resa
	{
		...
		void ReservationsListInterfaceElement::storeParameters(ValueElementList& vel)
		{
			...
			_titleCellStyle = vel.front();
			_tableStyle = vel.front();
		}
		...
		string ReservationsListInterfaceElement::display(
			...
			string userName = _userName->getValue(parameters, variables, object, request);
			...
			if (!userName.empty())
			{
				...
				for (vector<shared_ptr<ReservationTransaction> >::const_iterator itr(resats.begin()); itr != resats.end(); ++itr)
				{
					...
					for (vector<shared_ptr<Reservation> >::const_iterator it(resas.begin()); it != resas.end(); ++it)
						_DisplayReservation(stream, t, it->get());
				}
...

			{
				...
				for	(vector<shared_ptr<ScheduledService> >::const_iterator it(services.begin()); it != services.end(); ++it)
				{
					...
					stream << t.row();
					stream << t.col(6, titleCellStyle) << "Service " << service->getServiceNumber() << " - départ de " << service->getDepartureSchedule().getHour().toString();
					...
					*/					// Dates of departure
					...
					{
						_DisplayReservation(stream, t, serviceReservations.getReservation(itr->get()).get());
					}
					/*				}
					else
					break;
		...
		void ReservationsListInterfaceElement::_DisplayReservation( std::ostream& stream, html::HTMLTable& t, const Reservation* reservation)
		{
			stream << t.row();
